...
|MSBuild automatycznie zużywa zestawy i pliki metadanych (.winmd) dla systemu Windows.|T|Odwołania do zestawu SDK są automatycznie przekazywane do kompilatora.|T||
|MSBuild automatycznie wykorzystuje pliki .h lub lib.|T|*Nazwazestawusdk*pliku .props informuje program Visual Studio sposobu konfigurowania katalogu Visual C++ i tak dalej, do automatycznego przez plik .h lub lib.|N||
|MSBuild automatycznie wykorzystuje pliki js i CSS.|T|W **Eksploratora rozwiązań**można rozwinąć węzła odwołanie JavaScript SDK w celu wyświetlania poszczególnych js i CSS pliki, a następnie wygenerować `<source include/>` tagi przeciągając tych plików do ich plików źródłowych. Zestaw SDK obsługuje F5 i automatyczne pakiet Instalatora.|T||
...
|Mechanizm można określić "nie do kopiowania."|T|W zależności od tego, czy pliki są usuwane w folderze \redist lub \designtime można kontrolować plików do skopiowania do pakietu odbierającą aplikację.|N|Należy zadeklarować plików do skopiowania w manifeście pakietu.|
...
|Mechanizm obsługuje określania informacji o rejestracji dla natywnych metadanych Winmd.|T|W SDKManifest.xml można określić korelacji między pliku winmd i pliku dll.|N||
...
|Mechanizm wdraża pliki bez odwołań (na przykład wdrożyć struktury testowej, na których można uruchamiać testy z [!INCLUDE[win8_appname_long](../debugger/includes/win8_appname_long_md.md)] aplikacji).|T|Jeśli zgubisz pliki w folderze \redist pliki są automatycznie wdrażane.|T||
...
|Mechanizm znajduje się plik .exe autonomicznej dotyczące tworzenia i używania pakietów.|T|Zestaw SDK zawiera MSBuild.exe.|T||
