...
1.  W formularzu, należy wybrać **Zamknij** przycisku. (Jeśli nadal masz wszystkie przyciski wybrana, wybierz klawisz ESC, aby anulować wybór). Przewiń w **właściwości** okna, aż zostanie wyświetlony **(nazwa)** właściwości. ( **(Nazwa)** właściwość jest u góry, gdy właściwości są alfabetycznym.) Zmień nazwę, aby **closeButton**, jak pokazano na poniższej ilustracji.  
  
     ![Okno właściwości o nazwie closeButton](../ide/media/express_setnameproperty.png "Express_SetNameProperty")  
Okno właściwości o nazwie closeButton  
...
4.  Skoncentruj się na tej części kodu. (Wybierz **VB** karcie poniżej, jeśli używasz programu Visual Basic, aby wyświetlić wersję języka Visual Basic kodu.)  
    ...
    > [!NOTE]
    >  W tym samouczku został uproszczony kod Visual Basic, który jest automatycznie generowany przez usunięcie wszystkich między nawiasy (). Jeśli ten problem wystąpi, należy usunąć ten sam kod. Program będzie działać w obu przypadkach. W pozostałej części samouczków automatycznie wygenerowanego kodu jest uproszczone, jeśli to możliwe.  
  
5.  Wybierz kartę Projektant formularzy systemu Windows ponownie (**pliku Form1.cs [projekt]** języka Visual C#, **Form1.vb [projekt]** w języku Visual Basic), a następnie otwórz plik kodowy dla **Wyczyść obraz** przycisk, aby utworzyć metodę dla niego w kodzie formularza. Powtórz tę czynność dla pozostałych dwóch przycisków. Zawsze, IDE dodaje nową metodę do pliku kodu formularza.  
  
6.  Aby dodać jedna metoda, otwórz plik kodu dla formantu pole wyboru w Projektant formularzy systemu Windows, aby dodać IDE `checkBox1_CheckedChanged()` metody. Ta metoda jest wywoływana, gdy użytkownik zaznaczy lub wyczyszczenie pola wyboru.  
  
    > [!NOTE]
    >  Podczas pracy w programie, często przechodzenie między edytora kodu i Projektant formularzy systemu Windows. IDE ułatwia nawigowanie w projekcie. Użyj **Eksploratora rozwiązań** otworzyć Projektant formularzy systemu Windows, klikając dwukrotnie **pliku Form1.cs** języka Visual C# lub **Form1.vb** w języku Visual Basic lub na pasku menu, wybierz **Widoku**, **projektanta**.  
     ...
     Po wyświetleniu kod dla formantu w IDE w czasie projektowania Visual Studio dodaje metoda obsługi zdarzeń dla formantu, jeśli jeden nie ma. Na przykład po dwukrotnym kliknięciu przycisku IDE dodaje obsługi zdarzenia jego kliknij (która jest wywoływana, gdy użytkownik wybierze przycisk). Dwukrotne kliknięcie pola wyboru IDE dodaje obsługi zdarzenia jego CheckedChanged (która jest wywoływana, gdy użytkownik zaznaczy lub czyści okno).  
