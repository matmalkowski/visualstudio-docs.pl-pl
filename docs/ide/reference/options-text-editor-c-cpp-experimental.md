...
## <a name="additional-features-in-the-visual-studio-gallery"></a>Dodatkowe funkcje w galerii programu Visual Studio
Dodatkowy tekst funkcje edycji w galerii programu Visual Studio, zapoznaj się z listą [tutaj](http://go.microsoft.com/fwlink/?LinkId=692016). Na przykład [C++ szybkie rozwiązania](https://visualstudiogallery.msdn.microsoft.com/be91feef-8dc3-4f7a-ac9f-f34e7ca5918f), który obsługuje następujące:  
...
-   **Pomoc MSDN** -MSDN wyszukiwania dla komunikaty o błędach  
  
Można albo umieść nad wężyk do pobierania żarówka lub użyj domyślnej skrótu klawiaturowego Ctrl + kropka (Ctrl +.). Należy pamiętać, że dla skrótu klawiaturowego Twojej karetkę nie musi się znajdować na określony błąd lub token; po prostu można w tym samym wierszu jako błąd do wywołania sugestie dla wszystkich elementów w tym wierszu.  
