...
Możesz przeglądać funkcje edycji dodatkowy tekst w [Visual Studio Marketplace](https://marketplace.visualstudio.com/search?target=VS&category=Tools&vsVersion=&subCategory=All&sortBy=Downloads). Na przykład [C++ szybkie rozwiązania](https://marketplace.visualstudio.com/items?itemName=VisualCPPTeam.CQuickFixes2017), który obsługuje następujące:
