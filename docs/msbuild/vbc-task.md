...
|`Deterministic`|Opcjonalne `Boolean` parametru.<br/><br/> Jeśli `true`, powoduje, że kompilator output zestawu, którego zawartość binarna jest identyczna w kompilacji, jeśli dane wejściowe są identyczne.<br/><br/>Aby uzyskać więcej informacji, zobacz [-deterministyczne](/dotnet/visual-basic/reference/compiler-line-compiler/deterministic).|
...
|`MainEntryPoint`|Opcjonalne `String` parametru.<br /><br /> Określa klasę lub moduł, który zawiera `Sub Main` procedury. Ten parametr corresonds do [-głównym](/dotnet/visual-basic/reference/command-line-compiler/main) przełącznika kompilatora vbc.exe.|  
...
|`Optimize`|Opcjonalne `Boolean` parametru.<br /><br /> Jeśli `true`, włącza optymalizacje kompilatora. Ten parametr odpowiada [-zoptymalizować](/dotnet/visual-basic/reference/command-line-compiler/optimize) przełącznika kompilatora vbc.exe.|  
...
|`Resources`|Opcjonalne <xref:Microsoft.Build.Framework.ITaskItem> `[]` parametru.<br /><br /> Osadza zasobu .NET Framework do pliku wyjściowego. Ten parametr odpowiada [-zasobów](/dotnet/visual-basic/reference/command-line-compiler/resource) przełącznika kompilatora vbc.exe.|  
