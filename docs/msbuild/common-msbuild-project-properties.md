...
|Właściwość BaseAddress|Określa adres bazowy zestawu wyjściowego głównego. Ta właściwość jest odpowiednikiem `/baseaddress` przełącznika kompilatora.|  
...
|NazwaKonteneraKlucza|Nazwa kontenera klucza silnej nazwy.|  
|KeyOriginatorFile|Nazwa pliku klucza silnej nazwy.|  
|NoWin32Manifest|Określa, czy kompilator generuje domyślnego manifestu Win32 w zestawie danych wyjściowych. Wartość domyślna `false` oznacza, że domyślnego manifestu Win32 jest generowane dla wszystkich aplikacji. Ta właściwość jest odpowiednikiem `/nowin32manifest` przełącznika kompilatora vbc.exe.|  
