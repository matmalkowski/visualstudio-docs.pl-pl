...
|---------------------|-----------------|--------------|
|Podczas ładowania pliku dyrektywy include "{0}" zwróciła wartość null lub pusty ciąg.|Występuje, gdy tekst dołączony plik szablonu jest pusty. Komunikat zawiera nazwę pliku dołączanego pliku.|Usuń dyrektywy include lub upewnij się, że plik ma zawartość.|
...
|Procesor o nazwie "{0}"nie można odnaleźć dla dyrektywy o nazwie"{1}". Dyrektywa zostanie zignorowana.|Występuje, gdy w systemie nie można odnaleźć `directive` procesora określona w `custom` dyrektywy. Komunikat zawiera nazwy dyrektywy, Nazwa procesora i numer wiersza dyrektywy.|Ustaw `processor` atrybutu w dyrektywie nazwę procesora dyrektywy.|
|Wymagany parametr "{0}"dla dyrektywy"{1}" nie został znaleziony. Dyrektywa zostanie zignorowana.|Występuje, gdy system nie ma wymaganego parametru dyrektywy. Komunikat zawiera nazwę brakującego parametru, dyrektywy nazwę i numer wiersza.|Podaj brakujący parametr.|
