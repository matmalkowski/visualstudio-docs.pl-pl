...
Interfejs do wyświetlania rekordów bazy danych w tym przewodniku jest podstawowy, i jest tworzona prawa w dokumencie. Jeden <xref:Microsoft.Office.Tools.Word.ContentControl> przedstawia rekord pojedynczej bazy danych w czasie i dwa <xref:Microsoft.Office.Tools.Word.Controls.Button> formanty umożliwiają i z powrotem przewijania rekordów. Formant zawartości używa <xref:System.Windows.Forms.BindingSource> do połączenia z bazą danych.
