...
#### <a name="remove-activex-wrappers-when-the-document-is-opened"></a>Po otwarciu dokumentu, Usuń otoki ActiveX  
 ...
 Należy pamiętać, że jeśli dodatek VSTO tworzy formantów dynamicznych po otwarciu dokumentu, dodatek VSTO już wywoła `GetVstoObject` metody w ramach procesu tworzenia kontrolki. Nie trzeba dodać wymaga oddzielnego wywołania do `GetVstoObject` metodę, aby usunąć otoki ActiveX w tym scenariuszu.  
