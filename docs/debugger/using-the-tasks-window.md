...
> [!NOTE]
> W kodzie zarządzanym <xref:System.Threading.Tasks.Task> mający stan [TaskStatus.Created](<xref:System.Threading.Tasks.TaskStatus.Created>), [TaskStatus.WaitingForActivation](<xref:System.Threading.Tasks.TaskStatus.WaitingForActivation>), lub [TaskStatus.WaitingToRun](<xref:System.Threading.Tasks.TaskStatus.WaitingToRun>) może nie być wyświetlane w **zadania** okna wątki zarządzane są w stanie uśpienia lub sprzężenia.
...
|**task_group**|Dla kodu natywnego adresu [task_group](/cpp/parallel/concrt/reference/task-group-class) obiekt, który zaplanowane zadanie. Dla agentów asynchronicznych i zadań lekkich w tej kolumnie jest równa 0.|
