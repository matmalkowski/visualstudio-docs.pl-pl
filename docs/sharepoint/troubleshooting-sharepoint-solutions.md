...
ms.locfileid: "37120312"
---
# <a name="troubleshoot-sharepoint-solutions"></a>Rozwiązywanie problemów z rozwiązań SharePoint
...
## <a name="user-must-refresh-application-page-in-browser-while-debugging-to-view-updated-image"></a>Użytkownik należy odświeżyć stronę aplikacji w przeglądarce podczas debugowania, aby wyświetlić zaktualizowane obrazy
...
-   Upewnij się, że masz dostęp administratora do witryny sieci Web programu SharePoint. Aby uzyskać więcej informacji, zobacz [!INCLUDE[TLA2#tla_office](../sharepoint/includes/tla2sharptla-office-md.md)] Online artykułu [Przypisywanie lub usuwanie administratorów usługi aplikacji w programie SharePoint Server](https://docs.microsoft.com/en-us/sharepoint/administration/assign-or-remove-administrators-of-service-applications).  
...
1.  W **Eksploratora rozwiązań**, otwórz odbiorcy zdarzeń *.feature* w pliku **projektanta funkcji** przez dwukrotne kliknięcie pliku lub otwieranie menu skrótów, a następnie Wybieranie **Otwórz**.  
...
## <a name="error-appears-when-a-renamed-imported-list-instance-is-run"></a>Błąd pojawia się po zaimportowaniu zmienionej nazwie wykonywania wystąpienia listy
...
### <a name="resolution"></a>Rozwiązanie  
 Po zaimportowaniu wystąpienia listy atrybut o nazwie CustomSchema jest dodane do pliku Elements.xml wystąpienia listy. Elements.XML zawiera ścieżkę niestandardowego pliku schema.xml dla wystąpienia listy. Jeśli zmienisz nazwę wystąpienia listy w [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)], ścieżkę wdrażania dla niestandardowego pliku schema.xml zmian, ale wartość atrybutu CustomSchema ścieżki nie jest aktualizowana. W związku z tym nie można odnaleźć wystąpienia listy *schema.xml* pliku w starej ścieżki, który jest określony przez atrybut CustomSchema, gdy funkcja jest aktywna.  
  
 Aby rozwiązać ten problem, należy zaktualizować ścieżki lokalizacji wdrożenia *schema.xml* pliku w atrybucie CustomSchema.  
...
## <a name="sharepoint-debugging-session-terminated-by-iis"></a>Programu SharePoint został przerwany przez usługi IIS sesji debugowania
 Ten problem występuje, gdy Ustaw punkt przerwania [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] rozwiązania programu SharePoint, wybierz **F5** klawisz, aby uruchomić go, a następnie zachować w punkcie przerwania dłużej niż 90 sekund.  
...
## <a name="auto-retract-leaves-orphaned-list-instance-in-sharepoint"></a>Wycofać automatycznie pozostawia wystąpienie listy oddzielone w programie SharePoint
...
### <a name="resolution"></a>Rozwiązanie  
 ...
 Aby rozwiązać ten problem, należy wdrożyć rozwiązanie, z menu, wybierając **kompilacji** > **Wdróż**. (Nie debugowania rozwiązania, wybierając **F5** klucza.) Następnie należy usunąć wystąpienia listy w programie SharePoint.  
  
## <a name="original-sharepoint-solution-is-replaced-by-an-exported-version"></a>Oryginalny rozwiązania SharePoint zastępuje eksportowanych wersji
