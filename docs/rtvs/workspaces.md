...
Domyślnie RTVS nie zostanie zapisany stan obszaru roboczego po zamknięciu i ponownie otwórz projekt. Możesz zmienić to zachowanie, jednak za pomocą [Opcje obszaru roboczego](options.md#workspace).
...
Zdalnych obszarów roboczych pozwalają połączyć się z sesją R na komputerze zdalnym. (Zobacz [Konfigurowanie zdalnych obszarów roboczych](workspaces-remote-setup.md) dotyczące sposobu konfigurowania komputera do tego celu.)
...
> [!Tip]
> Aby wyłączyć zapisywanie monitu, wybierz pozycję **R Narzędzia > Opcje** polecenia i ustaw **Pokaż okno dialogowe potwierdzenia przed przełączeniem obszarów roboczych** opcji w celu `No`. Zobacz [Opcje obszaru roboczego](options.md#workspace).
...
Jednak to ostrzeżenie pojawia się również czy *certyfikatu z podpisem własnym* użyto, aby włączyć protokół HTTPS na komputerze zdalnym, zamiast korzystać z zaufanego dostawcę. Aby uzyskać więcej informacji, zobacz [Konfigurowanie zdalnych obszarów roboczych](workspaces-remote-setup.md).
