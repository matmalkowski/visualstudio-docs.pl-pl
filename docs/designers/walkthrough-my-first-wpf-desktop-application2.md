...
2.  W **nowy projekt** okna dialogowego, rozwiń pozycję **Visual C#** lub **Visual Basic** węzła i wybierz polecenie **Windows** węzeł, a następnie rozwiń węzeł **Windows** węzeł i wybierz polecenie **klasyczny pulpit** węzła.  
  
3.  Na liście szablonów wybierz **aplikacji WPF** szablonu.  
  
4.  W **nazwa** wprowadź pole tekstowe `ExpenseIt`, a następnie wybierz pozycję **OK** przycisku.  
...
2.  Jeśli używasz programu C#, wyszukaj wiersz `<Window x:Class="ExpenseIt.MainWindow"` i zastąpić go ciągiem `<NavigationWindow x:Class="ExpenseIt.MainWindow"`.  
...
3.  Wybierz `<Grid>` i `</Grid>` znaczniki i usuń je.  
  
     A **NavigationWindow** nie może zawierać inne elementy interfejsu użytkownika, takich jak **siatki**.  
  
4.  W **właściwości** okna, rozwiń węzeł **typowe** węzła kategorii i wybierz polecenie **tytuł** właściwości, a następnie wprowadź `ExpenseIt` i naciśnij klawisz **Enter**  klucza.  
...
5.  W oknie XAML, ustaw wartość **wysokość** elementu `375`i ustaw wartość **szerokość** właściwości `500`.  
...
2.  Znajdź wiersz `public partial class MainWindow : Window` i zastąpić go ciągiem `public partial class MainWindow : NavigationWindow`.  
...
2.  W **Dodaj nowy element** okno dialogowe, wybierz **nazwa** tekstu i wpisz `ExpenseItHome`, a następnie wybierz pozycję **Dodaj** przycisku.  
  
     Ta strona jest pierwszym oknie, która jest wyświetlana, gdy aplikacja jest uruchamiana.  
  
3.  W projektancie, wybierz **ExpenseItHome.xaml** karcie, jeśli nie jest jeszcze aktywne Karta konstruktora.  
  
4.  Wybierz `<Title>` element i Zmień tytuł, aby **programu ExpenseIt — strona główna**.  
...
6.  Znajdź wiersz `Title="ExpenseIt" Height="375" Width="500">` element i Dodaj `Source="ExpenseItHome.xaml"` właściwości.  
...
2.  W **Dodaj nowy element** okno dialogowe, wybierz **nazwa** tekstu i wpisz `ExpenseReportPage`, a następnie wybierz pozycję **Dodaj** przycisku.  
  
     To okno wyświetli raport poszczególnych wydatków.  
  
3.  W projektancie, wybierz **ExpenseReportPage.xaml** karcie, jeśli nie jest jeszcze aktywne Karta konstruktora.  
  
4.  Wybierz `<Title>` element i Zmień tytuł, aby **programu ExpenseIt — widok wydatków**.  
...
5.  Na pasku menu wybierz **debugowania**, **Rozpocznij debugowanie** (lub naciśnij klawisz F5) do uruchomienia aplikacji.  
...
2.  W **właściwości** okna, rozwiń węzeł **układu** węzła kategorii i zestawu **margines** wartości do `10`, `10`, `0`i `10`, które odpowiada lewo, prawo, górny i dolny marginesy.  
...
3.  Dodaj następujący kod XAML, aby `Grid` element do tworzenia definicji wierszy i kolumn:  
...
2.  Dodaj następujący kod XAML tylko powyżej `</Grid>` tag, aby utworzyć `Border`, `ListBox` i `Button` kontrolki:  
...
3.  Skompiluj i uruchom aplikację. Na poniższej ilustracji przedstawiono wyglądu w czasie wykonywania formantów, które zostały utworzone przy użyciu języka XAML w tej procedurze.  
...
2.  W **Eksploratora rozwiązań**, otwórz menu skrótów **programu ExpenseIt** węzeł i wybierz polecenie **Dodaj**, **istniejący element**.  
  
3.  W **Dodaj istniejący element** okna dialogowego, Znajdź **watermark.png** obrazu, który właśnie został dodany, wybierz go, a następnie wybierz pozycję **Dodaj** przycisku.  
  
    > [!NOTE]
    >  Konieczne może być rozwiń **typów plików** listę i wybierz **pliki obrazów**.  
  
4.  Otwórz **ExpenseItHome.xaml** pliku i Dodaj następujący kod XAML tylko powyżej `</Grid>` tag, aby utworzyć obraz tła:  
...
4.  Przenieś formanty do drugiej kolumny przez ustawienie `Grid.Column` do wartości 1. Przenieś każdego formantu w dół wiersza, zwiększając każdego `Grid.Row` wartość 1.  
    ...
    2.  Znajdź wiersz `<ListBox Name="peopleListBox" Grid.Column="0" Grid.Row="1"`. Zmień `Grid.Column="0"` do `Grid.Column="1"` i zmienić `Grid.Row="1"` do `Grid.Row="2"`.  
  
    3.  Znajdź wiersz `<Button Grid.Column="0" Grid.Row="2" Margin="0,10,0,0" Width="125"`. Zmień `Grid.Column="0"` do `Grid.Column="1"` i zmienić `Grid.Row="2"` do `Grid.Row="3"`.  
...
6.  Jeśli skompilować i uruchomić aplikację w tym momencie, powinien wyglądać jak na poniższej ilustracji:  
...
2.  Wybrana opcja `Button` element i Dodaj następujący kod XAML natychmiast po `HorizontalAlignment="Right"` element: `Click="Button_Click"`.  
...
1.  Otwórz **ExpenseReportPage.xaml**.  
     ...
     Ta strona przedstawia raportu wydatków osoby, która jest zaznaczona na stronie głównej.  
  
2.  Dodaj następujący kod XAML między `<Grid>` i `</Grid>` tagów:  
...
2.  Dodaj następujące XAML między `<Application.Resources>` i `</Application.Resources>` tagów:  
...
3.  Otwórz **ExpenseItHome.xaml** i Zastąp wszystko pomiędzy `<Grid>` i `</Grid>` elementy o następujących XAML:  
...
4.  Otwórz **ExpenseReportPage.xaml** i Zastąp wszystko pomiędzy `<Grid>` i końcowych `</Grid>` elementy o następujących XAML:  
...
4.  Zastąp istniejące `<ListBox>` elementu o następujących XAML:  
...
2.  W języku C#, Dodaj następujący Konstruktor do **ExpenseReportPage** klasy lub w języku Visual Basic Zastąp istniejące klasy następującym kodem:  
...
4.  Zastąp `Click` kod obsługi zdarzenia z następujących czynności:  
...
2.  Zastąp kod XAML **nazwa** i **działu** `<StackPanel` elementy z następujących czynności:  
...
4.  Zastąp `<DataGrid>` element z następujących czynności:  
