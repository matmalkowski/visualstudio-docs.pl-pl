...
Możesz też sprawdzić za pomocą poświadczeń `az webapp deployment list-publishing-profiles` polecenia za pomocą wiersza polecenia platformy Azure (zobacz [wdrożenia aplikacji sieci Web az](/cli/azure/webapp/deployment?view=azure-cli-latest#az-webapp-deployment-list-publishing-profiles)). Biblioteka pomocnika do ogłaszania Kudu poleceń jest dostępna na [GitHub](https://github.com/lmazuel/azure-webapp-publish/blob/master/azure_webapp_publish/kudu.py#L42).
