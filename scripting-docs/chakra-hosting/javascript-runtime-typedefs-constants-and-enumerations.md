---
title: Definicje typów środowiska wykonawczego języka JavaScript, stałe i wyliczenia | Dokumentacja firmy Microsoft
ms.custom: ''
ms.date: 01/18/2017
ms.prod: windows-client-threshold
ms.reviewer: ''
ms.suite: ''
ms.tgt_pltfrm: ''
ms.topic: reference
ms.assetid: 1aa107ed-e144-4947-b5bb-90284a537174
caps.latest.revision: 5
author: mikejo5000
ms.author: mikejo
manager: ghogen
ms.openlocfilehash: ad2f5a086062d6a4473a1befc83790d76056d19c
ms.sourcegitcommit: aadb9588877418b8b55a5612c1d3842d4520ca4c
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 10/27/2017
ms.locfileid: "24788623"
---
# <a name="javascript-runtime-typedefs-constants-and-enumerations"></a>Typedefy, stałe i wyliczenia środowiska uruchomieniowego JavaScript
Definicje typów środowiska wykonawczego języka JavaScript (JsRT), stałe i wyliczenia obsługuje dodawanie funkcji obsługi skryptów do pulpitów i aplikacji po stronie serwera z systemem Windows.  
  
## <a name="in-this-section"></a>W tej sekcji  
 Następujące globalne definicje typów obsługi JsRT:  
  
-   [JsBackgroundWorkItemCallback — Typedef](../chakra-hosting/jsbackgroundworkitemcallback-typedef.md)  
  
-   [JsBeforeCollectCallback — Typedef](../chakra-hosting/jsbeforecollectcallback-typedef.md)  
  
-   [JsContextRef — Typedef](../chakra-hosting/jscontextref-typedef.md)  
  
-   [JsFinalizeCallback — Typedef](../chakra-hosting/jsfinalizecallback-typedef.md)  
  
-   [JsMemoryAllocationCallback — Typedef](../chakra-hosting/jsmemoryallocationcallback-typedef.md)  
  
-   [JsNativeFunction — Typedef](../chakra-hosting/jsnativefunction-typedef.md)  
  
-   [JsObjectBeforeCollectCallback — Typedef](../chakra-hosting/jsobjectbeforecollectcallback-typedef.md)  
  
-   [JsProjectionCallback — Typedef](../chakra-hosting/jsprojectioncallback-typedef.md)  
  
-   [JsProjectionCallbackContext — Typedef](../chakra-hosting/jsprojectioncallbackcontext-typedef.md)  
  
-   [JsProjectionEnqueueCallback — Typedef](../chakra-hosting/jsprojectionenqueuecallback-typedef.md)  
  
-   [JsPromiseContinuationCallback — Typedef](../chakra-hosting/jspromisecontinuationcallback-typedef.md)  
  
-   [JsPropertyIdRef — Typedef](../chakra-hosting/jspropertyidref-typedef.md)  
  
-   [JsRef — Typedef](../chakra-hosting/jsref-typedef.md)  
  
-   [JsRuntimeHandle — Typedef](../chakra-hosting/jsruntimehandle-typedef.md)  
  
-   [JsSerializedScriptLoadSourceCallback — Typedef](../chakra-hosting/jsserializedscriptloadsourcecallback-typedef.md)  
  
-   [JsSerializedScriptUnloadCallback — typedef](../chakra-hosting/jsserializedscriptunloadcallback-typedef.md)  
  
-   [JsSourceContext — Typedef](../chakra-hosting/jssourcecontext-typedef.md)  
  
-   [JsThreadServiceCallback — Typedef](../chakra-hosting/jsthreadservicecallback-typedef.md)  
  
-   [JsValueRef — Typedef](../chakra-hosting/jsvalueref-typedef.md)  
  
 Następujące ograniczenia obsługi JsRT:  
  
-   [Js_invalid_propertyid — stała](../chakra-hosting/js-invalid-propertyid-constant.md)  
  
-   [Js_invalid_reference — stała](../chakra-hosting/js-invalid-reference-constant.md)  
  
-   [Js_invalid_runtime_handle — stała](../chakra-hosting/js-invalid-runtime-handle-constant.md)  
  
-   [Js_source_context_none — stała](../chakra-hosting/js-source-context-none-constant.md)  
  
 Następujące wyliczenia obsługi JsRT:  
  
-   [Jserrorcode — wyliczenie](../chakra-hosting/jserrorcode-enumeration.md)  
  
-   [Jsmemoryeventtype — wyliczenie](../chakra-hosting/jsmemoryeventtype-enumeration.md)  
  
-   [Jspropertyidtype — wyliczenie](../chakra-hosting/jspropertyidtype-enumeration.md)  
  
-   [Jsruntimeattributes — wyliczenie](../chakra-hosting/jsruntimeattributes-enumeration.md)  
  
-   [Jsruntimeversion — wyliczenie](../chakra-hosting/jsruntimeversion-enumeration.md)  
  
-   [Jstypedarraytype — wyliczenie](../chakra-hosting/jstypedarraytype-enumeration.md)  
  
-   [Jsvaluetype — wyliczenie](../chakra-hosting/jsvaluetype-enumeration.md)  
  
## <a name="see-also"></a>Zobacz też  
 [Hostowanie środowiska uruchomieniowego JavaScript](../chakra-hosting/hosting-the-javascript-runtime.md)   
 [Hostowanie środowiska uruchomieniowego JavaScript](../chakra-hosting/javascript-runtime-hosting.md)